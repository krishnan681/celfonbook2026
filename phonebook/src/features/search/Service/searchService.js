...
//       const term = (filters.businessName || filters.keywords)
//         .trim()
//         .replace(/,/g, "");
...
//     if (filters.letter) {
//       query = query.ilike("business_name", `${filters.letter.toUpperCase()}%`);
//     }
...
export const SearchService = {
  async logSearch(query, filter) {
    try {
      ...
      if (error) {
        console.error("Search Log Error:", error.message);
      }
    } catch (err) {
